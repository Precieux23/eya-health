...
export default function UserTable({ users = [], onDeleted, onEdit }) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error(err);
      alert(err?.response?.data?.message || "Erreur suppression utilisateur");
    } finally {
      setLoadingId(null);
    }
  };
    ...
    <div className="overflow-x-auto bg-white rounded-2xl shadow-lg border border-blue-100">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gradient-to-r from-blue-600 to-green-600 text-white">
          <tr>
            <th className="p-4 text-left font-semibold flex items-center gap-2"><User size={16} /> Nom</th>
