...
import { Plus, ArrowLeft, Edit, X, Package, Sparkles, CheckCircle, Activity } from "lucide-react";
...
export default function ProduitsPage() {
  ...
  const handleSuccessEdit = (updatedProd) => {
    if (!updatedProd) {
      setEditing(null);
      setIsSubmitting(false);
      setSubmitMessage("");
    ...
    setProduits((prev) => prev.map((p) => (p._id === updatedProd._id ? updatedProd : p)));
    setEditing(null);
    setIsSubmitting(false);
    setSubmitMessage("");
    setShowSuccess(true);
        ...
        {editing && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center overflow-y-auto z-50 animate-fadeIn">
            <div className="mt-28 mb-8 bg-white/95 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-2xl max-w-4xl w-full mx-4 relative border border-gray-200 transform animate-scaleIn h-fit">
              ...
              <div className="relative">
                <ProductForm initial={editing} onSuccess={(updatedProd) => {
                  if (updatedProd) {
                    setIsSubmitting(true);
                    setSubmitMessage("Modification en cours...");
                    setTimeout(() => {
                      setSubmitMessage("Redirection...");
                      handleSuccessEdit(updatedProd);
                    }, 1000);
                  } else {
          ...
          {/* Carte Disponibilité */}
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl border border-gray-200 hover:shadow-2xl transition-all duration-500 hover:scale-105 hover:-translate-y-1 group">
              <div className="flex items-center justify-between">
                <div className="w-14 h-14 bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-gray-600 font-semibold">Disponibilité</div>
                </div>
              </div>
