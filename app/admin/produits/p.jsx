...
import { Plus, ArrowLeft, Edit, X, Package, Sparkles } from "lucide-react";
...
export default function ProduitsPage() {
  ...
  const load = async () => {
    try {
      attachAuth();
      const res = await api.get("http://localhost:5000/api/produits");
      setProduits(res.data || []);
    } catch (err) {
      console.error(err);
      console.error(err?.response?.data?.message || "Impossible de charger les produits");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSuccessCreate = (newProd) => {
    if (!newProd) {
      setShowCreate(false);
      setEditing(null);
      return;
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between relative z-10">
          ...
          <nav className="hidden md:flex items-center gap-3">
            ...
            >
              ...
              <span className="relative z-10 font-semibold">Ajouter Produit</span>
      ...
      {/* CONTENU AVEC ESPACE POUR LE HEADER FIXE */}
      <div className="max-w-7xl mx-auto px-4 pt-20 relative z-10">
        {/* Overlay pour création avec animation */}
        {showCreate && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
            <div className="bg-white/95 backdrop-blur-xl p-8 rounded-3xl shadow-2xl max-w-4xl w-full mx-4 relative border border-gray-200 transform animate-scaleIn">
              {/* Décorations */}
              <div className="absolute -top-20 -right-20 w-40 h-40 bg-blue-400 rounded-full filter blur-3xl opacity-20"></div>
              <div className="absolute -bottom-20 -left-20 w-40 h-40 bg-purple-400 rounded-full filter blur-3xl opacity-20"></div>
              
              <button
                onClick={() => setShowCreate(false)}
                className="absolute top-6 right-6 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-full p-2 transition-all duration-300 hover:rotate-90"
              >
                <X size={24} />
              </button>
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-3xl font-bold bg-gradient-to-r from-blue-900 via-indigo-900 to-purple-900 bg-clip-text text-transparent">
                  Créer un Nouveau Produit
                </h3>
              </div>
              
              <div className="relative">
                <ProductForm onSuccess={handleSuccessCreate} />
              </div>
            </div>
          </div>
        )}

        {/* Overlay pour édition avec animation */}
        {editing && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
            <div className="bg-white/95 backdrop-blur-xl p-8 rounded-3xl shadow-2xl max-w-4xl w-full mx-4 relative border border-gray-200 transform animate-scaleIn">
              {/* Décorations */}
              <div className="absolute -top-20 -right-20 w-40 h-40 bg-indigo-400 rounded-full filter blur-3xl opacity-20"></div>
              <div className="absolute -bottom-20 -left-20 w-40 h-40 bg-pink-400 rounded-full filter blur-3xl opacity-20"></div>
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-3xl font-bold bg-gradient-to-r from-indigo-900 via-purple-900 to-pink-900 bg-clip-text text-transparent">
                  Modifier le Produit
                </h3>
              </div>
              
              <div className="relative">
                <ProductForm initial={editing} onSuccess={(res) => {
                  if (!res) {
                    setEditing(null);
                    return;
                  }
                  setProduits((prev) => prev.map((p) => (p._id === res._id ? res : p)));
                  setEditing(null);
                }} />
              </div>
            </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl border border-gray-200 hover:shadow-2xl transition-all duration-500 hover:scale-105 group">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-right">
                ...
                <div className="text-sm text-gray-600 font-medium">Produits</div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-right">
                <div className="text-3xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                  {produits.filter(p => p.quantite > 10).length}
                </div>
                <div className="text-sm text-gray-600 font-medium">En Stock</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl border border-gray-200 hover:shadow-2xl transition-all duration-500 hover:scale-105 group">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-right">
                ...
                <div className="text-sm text-gray-600 font-medium">Stock Bas</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mb-8 bg-white/80 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-gray-200 hover:shadow-3xl transition-all duration-500">
          <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 px-8 py-6">
            <div className="flex items-center gap-3">
              ...
              <div className="ml-auto px-4 py-2 bg-white/20 backdrop-blur-xl rounded-lg">
                <span className="text-white font-semibold">{produits.length} produits</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-16 space-y-4">
                ...
                <p className="text-gray-600 font-semibold animate-pulse">Chargement des produits...</p>
              </div>
            ) : (
              <ProductTable products={produits} onDeleted={handleDeleted} onEdit={handleEdit} />
            )}
          </div>
        </section>
      </div>
